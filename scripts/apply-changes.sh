#!/bin/bash
# Script to apply Liquibase changes
...
if [[ "$ENV" != "local-dev" && "$ENV" != "dev" ]]; then
  ...
  if aws s3 ls "s3://$S3_BUCKET/$S3_OBJECT_KEY" --profile your-aws-profile > /dev/null 2>&1; then
    ...
    if [[ $CURRENT_EPOCH -ge $START_EPOCH && $CURRENT_EPOCH -le $END_EPOCH ]]; then
      echo "Current time is within the allowed execution window. Proceeding with Liquibase update..."
    else
...
liquibase --defaultsFile="$PROPERTIES_FILE" update
